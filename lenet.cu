...
struct ConvBiasLayer
{
		...
		pbias(out_channels_)
	{
		...
		out_height = in_h_ - kernel_size_ + 1;   // strides = 1로 가정함.
	}
...
struct TrainingContext
{
	...
	cublasHandle_t cublasHandle;  // cublas를 사용하기 위한 핸들러

	cudnnTensorDescriptor_t dataTensor, conv1Tensor, conv1BiasTensor, pool1Tensor,
		conv2Tensor, conv2BiasTensor, pool2Tensor, fc1Tensor, fc2Tensor;  // 데이터 구조체 선언
	cudnnFilterDescriptor_t conv1filterDesc, conv2filterDesc;  // 필터 구조체 선언
	cudnnConvolutionDescriptor_t conv1Desc, conv2Desc;  // 컨볼루션 구조체 선언
	cudnnConvolutionFwdAlgo_t conv1algo, conv2algo; // 포워드알고리즘 구조체
	cudnnConvolutionBwdFilterAlgo_t conv1bwfalgo, conv2bwfalgo; // 백워드필터 알고리즘 구조체
	cudnnConvolutionBwdDataAlgo_t conv2bwdalgo;  // 백워드 데이터 구조체
	cudnnPoolingDescriptor_t poolDesc; // 풀링구조체 선언
	cudnnActivationDescriptor_t fc1Activation; // 활성함수 구조체 선언
		...
		FullyConnectedLayer& fc1, FullyConnectedLayer& fc2) : ref_fc1(fc1), ref_fc2(fc2), m_gpuid(gpuid)
	{
		...
		checkCUDNN(cudnnCreateTensorDescriptor(&dataTensor)); // input
		checkCUDNN(cudnnCreateTensorDescriptor(&conv1Tensor)); // conv1
		checkCUDNN(cudnnCreateTensorDescriptor(&conv1BiasTensor)); // conv1 bias
		checkCUDNN(cudnnCreateTensorDescriptor(&pool1Tensor)); // pool1
		checkCUDNN(cudnnCreateTensorDescriptor(&conv2Tensor)); // conv2 
		checkCUDNN(cudnnCreateTensorDescriptor(&conv2BiasTensor)); // conv2 bias
		checkCUDNN(cudnnCreateTensorDescriptor(&pool2Tensor)); // pool2
		checkCUDNN(cudnnCreateTensorDescriptor(&fc1Tensor)); //fully connect1
		checkCUDNN(cudnnCreateTensorDescriptor(&fc2Tensor)); // fully connect2

		checkCUDNN(cudnnCreateActivationDescriptor(&fc1Activation)); // activfunction
		...
		checkCUDNN(cudnnCreateConvolutionDescriptor(&conv1Desc)); // conv1 구조체
		checkCUDNN(cudnnCreateConvolutionDescriptor(&conv2Desc)); // conv2 구조체

		checkCUDNN(cudnnCreatePoolingDescriptor(&poolDesc));  // pooling 구조체


															  // Set tensor descriptor sizes
		checkCUDNN(cudnnSetTensor4dDescriptor(conv1BiasTensor, // conv1 bias tensor
			CUDNN_TENSOR_NCHW, // NCHW
			CUDNN_DATA_FLOAT,  // float
			1, conv1.out_channels, // batch_count, channel
			1, 1));					// height, width
		checkCUDNN(cudnnSetTensor4dDescriptor(conv2BiasTensor, // conv2 bias 구조
			CUDNN_TENSOR_NCHW,
			CUDNN_DATA_FLOAT,
			1, conv2.out_channels, // N, C
			1, 1));  // H, W

		checkCUDNN(cudnnSetPooling2dDescriptor(poolDesc, //pooling 결과 구조 확인
			CUDNN_POOLING_MAX, // set pooling mode
			CUDNN_PROPAGATE_NAN, // maxpooling Nan propagation mode
			pool1.size, pool1.size, // pooling window height, width
			0, 0,					// padding height, widht
			pool1.stride, pool1.stride));  // stride vertical, horizontal
		checkCUDNN(cudnnSetTensor4dDescriptor(pool2Tensor, // 2번째 pooling결과 셋업
			CUDNN_TENSOR_NCHW,
			CUDNN_DATA_FLOAT,
			batch_size, conv2.out_channels,  // N, C
			conv2.out_height / pool2.stride, // H
			conv2.out_width / pool2.stride)); // W
		...
		checkCUDNN(cudnnSetActivationDescriptor(fc1Activation, CUDNN_ACTIVATION_RELU,
			CUDNN_PROPAGATE_NAN, 0.0)); // 활성함수 Relu 셋업
		...
		float *pfc2, float *pfc2bias, void *workspace, float *onevec)
	{
		...
		// Conv1 layer
		checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, dataTensor, // cudnn핸들러, alpha, data구조체
			data, conv1filterDesc, pconv1, conv1Desc,  // 데이터, 필터 구조체, device 필터 데이터, 컨볼루션 구조체
			conv1algo, workspace, m_workspaceSize, &beta, //알고리즘, 버퍼 메모리, 알고리즘 사용시 버퍼 데이터 크기, beta
			conv1Tensor, conv1));  // 출력 구조체, device 출력 데이터
		...
		checkCudaErrors(cublasSgemm(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N, // cublas핸들러, transpose, nontranspose
			ref_fc1.outputs, m_batchSize, ref_fc1.inputs, // A C의 row , B C의 coulmn, A column and B row
			&alpha,						// scalar used for multiplication
			pfc1, ref_fc1.inputs,       // A행렬, 행렬A를 저장하는데 사용하는 2차원 배열
			pool2, ref_fc1.inputs,      // B행렬, 행렬B를 저장하는데 사용하는 2차원 배열
			&beta,						// beta값
			fc1, ref_fc1.outputs));     // C행렬, 행렬C를 저장하는데 사용하는 2차원 배열
		...
		checkCudaErrors(cublasSgemm(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N, // cublas핸들러, transpose, non transpose
		...
		void *workspace, float *onevec)
	{
		...
		// x[j] = alpha * x[j] for i=1,2,....,n and j = 1+(i-1)*incx
		checkCudaErrors(cublasSscal(cublasHandle,  // cublas 핸들러 
			ref_fc2.outputs * m_batchSize,         // scalar값으로 행렬연산에 사용(alpha).
			&scalVal,				// 벡터 x의 원소 개수 n개
			dloss_data, 1));		// 벡터 x. n개의 값이 있음, x의 strides(incx)
									// FC2 layer
...
int main(int argc, char **argv)
{
	...
	*/
	...
	ConvBiasLayer conv1((int)channels, 20, 5, (int)width, (int)height); // in_channel, out_channel, kernel_size, width, height
	MaxPoolLayer pool1(2, 2);  // size, stride
	ConvBiasLayer conv2(conv1.out_channels, 50, 5, conv1.out_width / pool1.stride, conv1.out_height / pool1.stride); // in_channel, out_channel, kernel_size, width, height
	MaxPoolLayer pool2(2, 2); // size, stride
	FullyConnectedLayer fc1((conv2.out_channels*conv2.out_width*conv2.out_height) / (pool2.stride * pool2.stride), //input
		500);																							// output
	FullyConnectedLayer fc2(fc1.outputs, 10);  // input, output

											   // Initialize CUDNN/CUBLAS training context
	TrainingContext context(FLAGS_gpu, FLAGS_batch_size, conv1, pool1, conv2, pool2, fc1, fc2); // gpuid, batch_size, .....

																								// Determine initial network structure
	bool bRet = true;
	if (FLAGS_pretrained) // 중간결과 저장
	{
	...
	//-----------------------------------------------------------------------------------------------------------------------------------------
	checkCudaErrors(cudaMalloc(&d_data, sizeof(float) * context.m_batchSize * channels           * height                            * width));  // 입력데이터 메모리 할당
	checkCudaErrors(cudaMalloc(&d_labels, sizeof(float) * context.m_batchSize * 1 * 1 * 1)); // 정답값 메모리 할당
	checkCudaErrors(cudaMalloc(&d_conv1, sizeof(float) * context.m_batchSize * conv1.out_channels * conv1.out_height                  * conv1.out_width)); //conv1메모리 할당
	...
	checkCudaErrors(cudaMalloc(&d_fc1relu, sizeof(float) * context.m_batchSize * fc1.outputs));   // fc1 활성함수 적용값 저장 메모리 할당
	checkCudaErrors(cudaMalloc(&d_fc2, sizeof(float) * context.m_batchSize * fc2.outputs));
	checkCudaErrors(cudaMalloc(&d_fc2smax, sizeof(float) * context.m_batchSize * fc2.outputs));   // 최종 softmax 할당

																								  // Network parameters
	float *d_pconv1, *d_pconv1bias, *d_pconv2, *d_pconv2bias;
	float *d_pfc1, *d_pfc1bias, *d_pfc2, *d_pfc2bias;

	checkCudaErrors(cudaMalloc(&d_pconv1, sizeof(float) * conv1.pconv.size())); // conv1.pconv의 size만큼 크기 할당
	checkCudaErrors(cudaMalloc(&d_pconv1bias, sizeof(float) * conv1.pbias.size())); // 바이어스크기만큼 할당
	...
	for (int iter = 0; iter < FLAGS_iterations; ++iter)
	{
		...
		checkCudaErrors(cudaMemcpyAsync(d_data, &train_images_float[imageid * context.m_batchSize * width*height*channels],
			sizeof(float) * context.m_batchSize * channels * width * height, cudaMemcpyHostToDevice)); // train image를 host->gpu로
	...
	if (classifications < 0)
		classifications = (int)test_size; // 10000
